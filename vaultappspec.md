...
> - **Pin the OAEP parameters explicitly, or reads fail on real hardware.** Android Keystore's OAEP uses an **SHA-1 MGF1** regardless of the `…OAEPWithSHA-256AndMGF1Padding` string, while the default software provider (used for public-key wrapping) disagrees — the mismatch throws `IllegalBlockSizeException` on unwrap. Both `wrap` and `unwrap` must init with an explicit `OAEPParameterSpec("SHA-256","MGF1",MGF1ParameterSpec.SHA1,PSpecified.DEFAULT)`, `wrap` must re-import the public key so encryption runs in the software provider, and the key must authorize **both** SHA-256 and SHA-1 digests. This never shows up in JVM/software-provider tests (both sides agree there) — only on-device. Caught on a Xiaomi during step-8 bring-up.
